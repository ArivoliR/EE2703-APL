def matrix_multiply(matrix1, matrix2):
    ...
    if not all(isinstance(item, int) for row in matrix1 for item in row):
        raise TypeError("All elements of matrix1 must be integers")

    if not all(isinstance(item, int) for row in matrix2 for item in row):
        raise TypeError("All elements of matrix2 must be integers")
